...
""" This Objects represent each N*N matrix which stores the matrix and its heuristic value of that particular matrix node """
class node:
    def __init__(self,state,h):
        self.state = state
        #self.g = g
        self.h = h
    # returns h(n) value of that particular puzzle node object.
    def get_h(self):
        return self.h
    # returns the matrix related to the particular node object.
    def get_state(self):
        return self.state

#returns possible child states for each queen    
...
"""returns the heuristic value of each matrix"""            
def heuristic(state):
...
"""returns the child_nodes of each matrix"""
...
"""Tperforms the side_way operations whenever it is stuck"""
def sideway(state):
    j=0
    for i in range(100):
        pos=0
...
"""performs the whole hill_climbing algorithm"""         
    ...
    while True:
        ...
        if state.get_h()==0:
             ...
             return j,True
                ...
                print("heuristic "+str(nxt_state.get_h()))
                state=nxt_state
                j+=1
            
    
            
"""used to create the random input_state"""
def get_Input(length,sample_size):
    ...
    print("Success_percent is = "+str(percent))
    print("Average_Number_of_success_steps"+str(np.average(avg_success_steps)))
    print("Average_number_of_failure_steps"+str(np.average(avg_failure_steps)))
    
    
sample_size = int(input("enter the number of samples to find the solution "))
